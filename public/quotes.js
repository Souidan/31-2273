...
var getQuoteFromJSON=function(index){
    ...
    var file=getQuotesFromJSON();
    if(arguments.length===1){
    var x=  getElementByIndexElseRandom(file,index);
    return x;
    }

    else {
        var x=  getElementByIndexElseRandom(file);
        return x;
...
exports.seed= function(cb){
    ...
    var collection=dbFile.db().collection('quotesCollection').count(function(err,count){
        if(count===0){
            dbFile.db().collection('quotesCollection').insert(getQuotesFromJSON(),function(err,docs){
